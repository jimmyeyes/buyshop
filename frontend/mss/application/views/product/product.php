...
//echo $sql;
        ...
        $data = array('temp1' => $temp1,'temp2' => $temp2,'brand' => $brand,'categoryid' => $categoryid, 'companyid' => $companyid, 'prodname' => $prodname, 'model' => $model, 'ean' => $ean, 'upc' => $upc, 'sku' => $sku, 'mpn' => $mpn, 'imei' => $imei, 'spec' => $spec, 'feture' => $feture);
        ...
        if (($authority & 128) == 128) {
         ...
         $data = array('shippingid' => $shippingid,  'currencyid' => $currencyid, 'amount' => $amount,'gram' => $gram,'temp1' => $temp1,'temp2' => $temp2,'brand' => $brand,'categoryid' => $categoryid, 'companyid' => $companyid, 'prodname' => $prodname, 'model' => $model, 'ean' => $ean, 'upc' => $upc, 'sku' => $sku, 'mpn' => $mpn, 'imei' => $imei, 'spec' => $spec, 'feture' => $feture);
...
//echo $sql;
	...
	function product_bath(){
			...
			if($ch==$ar){
				if($modtype==1 ){
                   ...
                   $row= $this->db->query($sql)->row();
                    ...
                    $sql="update  product set Quantity='".$Quantity."' ,avg='".$avg."' where productid='".$ar."'";
                    $this->db->query($sql);
        ...
        if($modtype==3){
            redirect('/product/product#tabs-3', 'refresh');
            return;
        }
